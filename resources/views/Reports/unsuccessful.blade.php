...
<div class="row mt-4 mb-2">
  <div class="col-sm-12">
        <h2 class="heder">Unsuccessful</h2>
    </div>
  <!-- <div class="col-sm-6 margin-tb">
   ...
   <!-- <form method="get" action="{{url('unsuccessful')}}" accept-charset="UTF-8">
...
<table class="table table-striped table-dark index-table mt-2">
 ...
 <th># </th>
   ...
   @if($counter==0)
        <tr> <td colspan="12" style="color: rgb(245, 165, 34);text-align:center;padding:10px;font-size:16px;"> No Records Found</td> </tr>
      @else  
        @foreach ($booking as $booking_key => $booking_value) 
          ...
          <td> {{ $booking_key+1 }}  </td>
            ...
            <td> {{ $booking_value->last_bid_price }}  </td>
            <td> {{ $booking_value->listing_price }}  </td>
            <td> {{ $booking_value->selling_price }}  </td>
            <td> {{ $booking_value->counter_value }}  </td>
...
<script>
    jQuery(document).ready(function($){
      ...
      $('.unsuccess').addClass('active-sub-menu');
